...
function KeyboardButtonBlock(
	...
	function handleSaveButton(): void {
		...
		if (rowIndex === null) {
			throw Error(
				'You call the save action, but rowIndex state must not be null.',
			);
		}

		if (buttonIndex === null) {
			throw Error(
				'You call the save action, but buttonIndex state must not be null.',
			);
		}
					...
					<Row className='g-2'>
						<Col xs={type === 'add' ? 12 : 6}>
							<Button
								...
								className='w-100'
								onClick={
									type === 'add' ? handleAddButton : handleSaveButton
								}
							>
